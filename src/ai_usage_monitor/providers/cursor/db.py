...
import sqlite3
from ai_usage_monitor.paths import get_cursor_db_path
...
def read_cursor_cached_email(db_path: Path | None = None) -> str | None:
  """Read cursorAuth/cachedEmail from Cursor's state.vscdb in read-only mode."""
  target_path = db_path or get_cursor_db_path()
  if not target_path.is_file():
    return None
  ...
  try:
    conn = sqlite3.connect(uri, uri=True, timeout=0.25)
    cursor = conn.cursor()
    cursor.execute("SELECT value FROM ItemTable WHERE key = ? LIMIT 1", ("cursorAuth/cachedEmail",))
    row = cursor.fetchone()
    if row and row[0] is not None:
      return _decode_sqlite_value(row[0])
  except Exception:
